...
function EnvDef($sid)
{
    ...
    $result = $xoopsDB->query('select idcat, nom, definition, lien from ' . $xoopsDB->prefix('glossaires') . " where id=$sid");
    ...
    <form action=\"glossaires-p-f.php\" method=post>
...
function MailDef($sid, $yname, $ymail, $fname, $fmail, $nom, $definition, $lien)
{
    ...
    $result2 = $xoopsDB->query('select idcat, nom, definition, lien from ' . $xoopsDB->prefix('glossaires') . " where id=$sid");
...
function ImprDef($sid)
{
    ...
    $hresult = $xoopsDB->query('SELECT commentaire, url FROM ' . $xoopsDB->prefix('glossaires_comm') . " WHERE def='$sid'");
    ...
    <a href="' . XOOPS_URL . '/modules/glossaires/">' . XOOPS_URL . '/modules/glossaires/</a>
    </td></tr></table><center>
	<FORM> 
