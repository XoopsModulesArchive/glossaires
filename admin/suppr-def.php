...
function SupprOk($id)
{
    ...
    $xoopsDB->query('DELETE FROM ' . $xoopsDB->prefix('glossaires') . " WHERE id = '$id'");

    $xoopsDB->query('DELETE FROM ' . $xoopsDB->prefix('glossaires_comm') . " WHERE def = '$id'");
...
function Suppr($id)
{
    ...
    $result = $xoopsDB->query('SELECT idcat, lettre, nom, definition, lien FROM ' . $xoopsDB->prefix('glossaires') . " WHERE id = '$id'");
