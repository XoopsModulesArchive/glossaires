...
function modifok($id, $idcat, $lettre, $nom, $definition, $lien)
{
    ...
    if ('' == $id) {
        ...
        $xoopsDB->query('UPDATE ' . $xoopsDB->prefix('glossaires') . " SET idcat= '$idcat', lettre = '$lettre' , nom = '$nom' , definition = '$definition', affiche='O', lien = '$lien'  WHERE id='$id'");
...
function modif($id)
{
    ...
    $TableRep = $xoopsDB->query('SELECT id, idcat, lettre, nom, definition, lien FROM ' . $xoopsDB->prefix('glossaires') . " WHERE id = '$id'");
