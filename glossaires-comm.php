...
function LirComm($sid)
{
    ...
    $TableRep = $xoopsDB->query('SELECT nom, idcat, definition, lien FROM ' . $xoopsDB->prefix('glossaires') . " WHERE affiche='O' AND id=$sid");
    ...
    if (0 == $NombreEntrees) {
        ...
        $result = $xoopsDB->query('SELECT id, auteur, date, commentaire, url FROM ' . $xoopsDB->prefix('glossaires_comm') . " where affiche='O' AND def=$sid ORDER BY date");
    ...
    if (1 == $anocomm || $xoopsUser) {
        echo " | <a href=\"glossaires-comm.php?sid=$sid\">" . _COMMADD . '</a>';
    }
...
function CoMm($sid, $mode)
{
    ...
    if (0 == $NombreEntrees) {
        ...
        $result = $xoopsDB->query('SELECT auteur, date, commentaire, url FROM ' . $xoopsDB->prefix('glossaires_comm') . " where affiche='O' AND def=$sid ORDER BY date");
    ...
    echo '<FORM ACTION="glossaires-comm.php" METHOD=POST>';
...
function AjComm($defs, $logname, $temp, $commentaire, $urls, $affiches)
{
    ...
    $xoopsDB->query('INSERT INTO ' . $xoopsDB->prefix('glossaires_comm') . " VALUES ('', '$defs', '$logname', '$temp', '$commentaire', '$urls', '$affiches')");
    ...
    if (1 == $mess) {
        ...
        $message = '' . _COMBY . " $logname\n\n" . XOOPS_URL . '/admin.php?fct=glossaires';
