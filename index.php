...
if ('Glossaires' == $xoopsConfig['startpage']) {
...
function Start()
{
    ...
    echo "<center><font><h2 class='bnewst'>" . _GLOSSAIRE . "</h2></font></center>\n";
    ...
    if ($xoopsUser) {
        if ($xoopsUser->isAdmin()) {
            ...
            [$propo] = $xoopsDB->fetchRow($xoopsDB->query('SELECT  COUNT(*)  FROM ' . $xoopsDB->prefix('glossaires') . " WHERE affiche='N' OR affiche='D' order by nom"));
            ...
            if (1 == $mode) {
                [$comm] = $xoopsDB->fetchRow($xoopsDB->query('SELECT  COUNT(*)  FROM ' . $xoopsDB->prefix('glossaires_comm') . " WHERE affiche='M' order by date"));
    ...
    [$numcats] = $xoopsDB->fetchRow($xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('glossaires_cat') . ''));
    ...
    $sresult = $xoopsDB->query('SELECT count(lien) as nbbbs FROM ' . $xoopsDB->prefix('glossaires') . " WHERE affiche='O' AND lien<>''");
    ...
    $hresult = $xoopsDB->query('SELECT count(id) as nbbs FROM ' . $xoopsDB->prefix('glossaires_comm') . " WHERE affiche='O'");
    ...
    echo "<table class='outer' width='50%' cellpadding='0' cellspacing'0' align='center'>
			<tr><td>
			<FORM ACTION=\"glossaires-rech.php\" METHOD=POST>";
    ...
    MenuGlo();	// Affichage du menu du glossaire
...
function Propose($nouvdef)
{
    ...
    if (1 == $nouvdef || $xoopsUser) {
        ...
        echo "<FORM ACTION=\"glossaires-ajout.php?pa=Propdef\" METHOD=POST>
...
function Demande()
{
    ...
    echo '<FORM ACTION="glossaires-ajout.php?pa=demdef" METHOD=POST><INPUT TYPE="hidden" NAME="affiche" VALUE="D"><INPUT TYPE="hidden" NAME="logname" VALUE="';
